...
// Helper: formatiranje JSON preporuke u HTML (za e‑mail)
...
function listHtml(arr){
  if (!arr || !arr.length) return "<div class='muted'>(nema)</div>";
  return `<ul>${arr.map(x => `<li>${escapeHtml(x)}</li>`).join("")}</ul>`;
}
function formatPreporukaAsHTML(p, opts = { includeHeader: true }){
  if(!p) return "<p>Nema preporuke.</p>";
  const naslov = escapeHtml(p.bolest || "—");
  return `
  <div style="font-family: Arial, Helvetica, sans-serif; line-height:1.5; color:#222">
    ${opts.includeHeader ? `<h2 style="margin:0 0 12px 0">Vaša biljna terapijska preporuka</h2>` : ""}
    ${opts.includeHeader ? `<p>Preporuka za: <strong>${naslov}</strong></p>` : ""}

    <h3 style="margin:16px 0 6px 0">🌿 Preporučene biljke</h3>
    ${listHtml(p.biljke)}

    <h3 style="margin:16px 0 6px 0">💊 Suplementi</h3>
    ${listHtml(p.suplementi)}

    <h3 style="margin:16px 0 6px 0">📚 Literatura</h3>
    ${listHtml(p.literatura)}

    ${p.napomena ? `<h3 style="margin:16px 0 6px 0">ℹ️ Napomena</h3><p>${escapeHtml(p.napomena)}</p>` : ""}

    <hr style="margin:20px 0; border:none; border-top:1px solid #eee" />
    <p style="font-size:12px; color:#666">Ova poruka je informativna i ne predstavlja medicinski savet.</p>
  </div>`;
}
...
const Analiza = () => {
  ...
  const handleSendEmail = async () => {
    try {
      const htmlBody = formatPreporukaAsHTML(preporukaJson, { includeHeader: false });
      const subj = `Biljna terapija – ${preporukaJson?.bolest || dijagnoza || "Preporuka"}`;
      ...
      const response = await fetch("http://localhost:8000/api/email/send", {
        ...
        body: JSON.stringify({
          email: user?.email,
          subject: subj,
          content: htmlBody  // ⬅ usklađeno sa backend modelom (izbegava 422)
        })
      });
      ...
      if (response.ok) {
        ...
        setPoruka("❌ Slanje mejla nije uspelo: " + msg);
      }
    } catch (error) {
      console.error("❌ Greška pri slanju mejla:", error);
      setPoruka("⚠️ Došlo je do greške pri slanju mejla: " + (error?.message || "Nepoznata greška"));
    }
  };
            ...
            <div className="overflow-y-auto max-h-80 text-gray-800">
              ...
              {preporukaJson ? (
                <div className="space-y-3">
                  ...
                  <div>
                    <div className="font-semibold">Reference i izvori</div>
